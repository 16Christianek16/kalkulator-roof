  ...
  return (
    <div>
      <button onClick={() => setOpen(o => !o)}
        ...
        style={{ color: 'rgba(255,255,255,0.35)', background: 'transparent', cursor: 'pointer', letterSpacing: '0.07em' }}>
...
export default function Sidebar({ open, onClose }) {
  return (
    <>
      ...
      <aside className={`fixed top-0 left-0 h-full w-56 z-30 flex flex-col transition-transform duration-300 lg:translate-x-0 lg:static lg:z-auto ${open ? 'translate-x-0' : '-translate-x-full'}`}
        ...
        <div className="flex items-center justify-between px-4 py-4 shrink-0"
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <div style={{ color: 'rgba(255,255,255,0.3)', fontSize: '0.65rem' }}>Stavební kalkulace</div>
            </div>
          </div>
        ...
        <nav className="flex-1 overflow-y-auto px-2 py-3 flex flex-col gap-0.5">
          {menu.map(item => <MenuItem key={item.label} item={item} onClose={onClose} />)}
        </nav>
